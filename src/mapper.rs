...
impl MapperType {
    ...
    pub fn write_mapper(&self, addr: u16, data: u8, mem: &mut[u8;0xffff], cart: &mut Cartridge) {
        ...
        match self {
            ...
            MapperType::MMC1 {
                mut mirroring,
                mut prg_rom_bank_mode,
                mut chr_rom_bank_mode,
                mut chr_bank0,
                mut chr_bank1,
                mut prg_bank,
                mut mmc1b,
                ...
                if (0x8000..=0x9fff).contains(&addr) {
                    if amount_shifted == 5 {
                        ...
                        chr_rom_bank_mode = if (shift_register & 0b1_0000) == 0b1_0000 {true} else {false};
                    ...
                    if amount_shifted == 5 {
                        ...
                        mmc1b = if shift_register & 0b1_0000 == 0b1_0000 {true} else {false};
                        prg_bank_changed = true;
                    }
                }
                if prg_bank_changed {
                    for i in 0x8000..=0xffff {
                        if prg_rom_bank_mode == 0 {
                            if i < 0xc000 {
                                mem[i] = cart.prg_rom_data[(i - 0x8000 + 16384 * (prg_bank & 0b1110) as usize) as usize];
                            } else {
                                mem[i] = cart.prg_rom_data[(i - 0x8000 + 16384 * (prg_bank & 0b1111) as usize) as usize];
                            }
                            ...
                            if i < 0xc000 {
                                mem[i] = cart.prg_rom_data[i - 0x8000]; // Set to first PRG bank
                            } else {
                                mem[i] = cart.prg_rom_data[(i - 0x8000 + 16384 * prg_bank as usize) as usize];
                            }
                        } else {
                            if i < 0xc000 {
                                mem[i] = cart.prg_rom_data[(i - 0x8000 + 16384 * prg_bank as usize) as usize];
                            } else {
                                mem[i] = cart.prg_rom_data[i - 0x8000 + cart.prg_rom_data.len() - 1 - 16384]; // Set to last PRG bank
                            }
                        }
                    }
                }
                ...
                //             if i < 0xb000 {
                //                 mem[i] = cart.prg_rom_data[(i - 0x8000 + 16384 * (prg_bank & 0b1110))];
                //             } else {
                //                 mem[i] = cart.prg_rom_data[(i - 0x8000 + 16384 * (prg_bank & 0b1111))];
                //             }
                //             //mem[i] = cart.prg_rom_data[(i-0x8000 + 32768 * (prg_bank & 0b1110) as usize) as usize]; // TODO see if this can work too
                //         } else if prg_rom_bank_mode == 1 {
                //             if i < 0xc000 {
...
#[cfg(test)]
mod mapper_tests {
    use crate::MapperType::{MMC1, Nrom};
    ...
    fn test_mmc1_write() {
        ...
        //     prg_data[i + 0x8000] = test_data;
        //}
        for i in 0x8000..0xffff {
            rom_data[i] = (i - 0x8000) as u8;
        }

        let mut test_cpu = Cpu6502 {
            a: 0,
            x: 0,
            y: 0,
            pc: 0x8000,
        ...
        mycpu.mapper.write_mapper(0x8000, 0b1111, &mut mycpu.cpu.mem, &mut mycpu.cartridge);
        mycpu.mapper.write_mapper(0x8000, 0b0111, &mut mycpu.cpu.mem, &mut mycpu.cartridge);
        match mycpu.mapper{
            MapperType::MMC1 {
                mirroring,
                prg_rom_bank_mode,
                ...
                assert_eq!(amount_shifted, 0b1000);
            }
